...
public class TestSegmentationModule extends GMPanel implements YModule, YObserver {
	...
	private class SampleSegThread extends SegmentingThread {
		...
		public void my_run() {
			...
			// this is for longer lasting methods that want to report progress
			// on the progress bar
			...
			// the voxel cube histogram keeps track of the occurence of all
			// values in the voxel cube
			...
			for (int z = z_percent; z < dim_z - z_percent; z++) {
				...
				// this will inform all observers (e.g. viewports) that the
				// segment has changed and shows the current progress
				_seg.new_data(true);
			}
